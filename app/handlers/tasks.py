...
async def add_task(messege:Message):
    await messege.answer("Выбирите действие",reply_markup=task_keyboard)

@tasks_router.message(F.text == "📋 Список задач")
async def get_task(messege:Message):
    await messege.answer("Выбирите действие",reply_markup=filter_keyboard)

@tasks_router.message(F.text == "◀️ Назад")
async def back(messege:Message):
    await messege.answer("Выбирите действие",reply_markup=main_kb)